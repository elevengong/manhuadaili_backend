...
class Xingyunfeiting extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'xingyunfeiting';
    ...
    {
        parent::__construct();
        $this->caiji_history = 'http://www.150557.com/draw-xyft.php';
        $this->caiji_plan = array(
            '28' => 'http://6hcc99.com/jihua/xingyunfeiting/xingyunfeitingiframe.php?p=gj',
            '29' => 'http://6hcc99.com/jihua/xingyunfeiting/xingyunfeitingiframe.php?p=yj',
            '30' => 'http://6hcc99.com/jihua/xingyunfeiting/xingyunfeitingiframe.php?p=jj'
        );
    }
    ...
    public function handle()
    {
        ...
        $writeContent = date('Y-m-d h:i:s',time()).'--开始采集幸运飞艇开奖记录start'."\r\n";
        ...
        if(empty($lottery))
        {
            $writeContent = date('Y-m-d h:i:s',time()).'--幸运飞艇不存在end'."\r\n";
            $this->writeLogs($writeContent,$logHistoryDir,$logHistoryName);
            exit;
        }
        if($lottery[0]['status'] == 0)
        {
            $writeContent = date('Y-m-d h:i:s',time()).'--幸运飞艇没有开启end'."\r\n";
            $this->writeLogs($writeContent,$logHistoryDir,$logHistoryName);
            exit;
        }
        $l_id = $lottery[0]['l_id'];
        ...
        $writeContent = date('Y-m-d h:i:s',time()).'--幸运飞艇开奖记录采集完成end'."\r\n";
        $this->writeLogs($writeContent,$logHistoryDir,$logHistoryName);

        //开启采集幸运飞艇的计划
        $writeContent = date('Y-m-d h:i:s',time()).'--开启采集幸运飞艇计划start'."\r\n";
        $this->writeLogs($writeContent,$logPlanDir,$logPlanName);

        $rules_plan = [
            'detail1' => ['#table-cq_ssc td:eq(2)','text'],
            'detail2' => ['#table-cq_ssc td:eq(3)','text']
        ];
        ...
        $writeContent = date('Y-m-d h:i:s',time()).'--幸运飞艇计划采集完成end'."\r\n";
        $this->writeLogs($writeContent,$logPlanDir,$logPlanName);
